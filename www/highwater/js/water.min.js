import{OCEAN_CONFIG as E}from"./config.min.js";const $=9.81,T=1e-6,ee=2,he=Object.freeze({swell:1,surge:0});export function bedHeightAt(a,t=E.beach){return Math.max(-t.maxDepth,(a-t.shoreZ)*t.slope)}export function tideOffset(a,t=E.water){return t.tideRange/2*Math.sin(Math.PI*2*a/t.tidePeriodSeconds)}export function depthAt(a,t,e=E.beach,n=E.water){const o=t-bedHeightAt(a,e);return o<=0?0:Math.max(n.minDepth,o)}export function waveNumberAt(a,t){if(t<=T)return a;const e=Math.tanh(a*t);return a/Math.sqrt(Math.max(e,T))}export function shoalingAt(a,t,e){if(t<=T||a<=T)return 1;const n=a*t,o=n>10?.5:.5*(1+2*n/Math.sinh(2*n)),r=Math.sqrt($/a*Math.tanh(n));return Math.sqrt(e/Math.max(2*o*r,T))}export function sharpenAt(a,t,e,n=E.water.crestSharpen){if(a<=T||t<=T||e<=T)return 0;const o=Math.tanh(t*e);if(o<=T)return n;const r=a*t/4*(3/(o*o*o)-1/o);return Math.min(n,r)}export function breakAmount(a,t,e=E.water){if(t<=T)return 1;const n=a/t,o=e.breakRatio-e.breakSoftness,r=e.breakRatio+e.breakSoftness;return P(o,r,n)}export function envelopeAt(a,t,e=E.water){const n=t*7.3,o=Math.sin(Math.PI*2*(a+n)/e.setPeriodSeconds),r=Math.sin(Math.PI*2*(a+n)/e.setSubPeriodSeconds);return 1+e.setDepth*o*r}export function rowPositions(a,t=E){const{beach:e,camera:n,water:o}=t,r=new Float32Array(a),i=Math.max(0,Math.min(e.nearRows,a-2)),l=a-i,g=n.z-e.nearZ,s=n.z-e.farZ;for(let u=0;u<i;u++)r[u]=n.z-(g+(e.rowNear-g)*(u/i));for(let u=0;u<l;u++){const w=l>1?u/(l-1):0;r[i+u]=n.z-(e.rowNear+(s-e.rowNear)*Math.pow(w,o.rowBias))}return r}export function halfWidthAt(a,t=E.beach,e=E.camera){return t.baseHalfWidth+Math.max(0,e.z-a)*t.widthPerMetre}export function waveConstants(a){return a.map(t=>{const e=Math.PI*2/t.length,n=Math.sqrt($/e),o=Math.max(-.95,Math.min(.95,t.dirX));return{k0:e,c0:n,omega:e*n*t.speed,kSin:e*o,amplitude:t.amplitude,steepness:t.steepness}})}export function buildProfile(a,t,e=E,n=null,o=he){const{beach:r,water:i}=e,l=a.length,g=waveConstants(i.waves),s=g.length,u=o.swell>0?o.swell:0,w=tideOffset(t,i)+o.surge,h=o.front||null,f=o.lull||null,d=n||{depth:new Float32Array(l),lift:new Float32Array(l),liftSlope:new Float32Array(l),breaking:new Float32Array(l),foamBed:new Float32Array(l),edge:new Float32Array(l),phase:new Float32Array(l*s),amp:new Float32Array(l*s),sharp:new Float32Array(l*s),k:new Float32Array(l*s)},x=[];for(let m=0;m<s;m++)x.push(envelopeAt(t,m,i));const W=4+40*i.foamPersistence,D=new Float32Array(s),I=new Float32Array(s);let K=0,V=0,ae=a[l-1];for(let m=l-1;m>=0;m--){const M=a[m],B=M-ae;ae=M;let _=w,j=0;h&&(j=1-P(h.z-h.width,h.z+h.width,M),_+=h.rise*j);let U=u;if(f){const p=P(f.z-f.width,f.z+f.width,M);U=u+(f.before-u)*p}h&&(U=U+(h.swellBehind-U)*j);let oe=w;if(h){const p=Math.max(0,h.z-M),F=h.body>0?1-P(h.body*.35,h.body,p):1;oe+=h.rise*j*F}d.lift[m]=oe;const C=depthAt(M,_,r,i);if(d.depth[m]=C,C<=0){d.breaking[m]=0,d.foamBed[m]=0,d.edge[m]=0;for(let p=0;p<s;p++)d.phase[m*s+p]=D[p],d.amp[m*s+p]=0,d.sharp[m*s+p]=0,d.k[m*s+p]=g[p].k0;continue}let Y=0;for(let p=0;p<s;p++){const F=g[p],Q=waveNumberAt(F.k0,C);d.k[m*s+p]=Q;const ie=X(F.kSin/Q,-.95,.95),Te=Math.sqrt(1-ie*ie);D[p]+=Q*Te*B,d.phase[m*s+p]=D[p];const xe=shoalingAt(Q,C,F.c0);I[p]=F.amplitude*U*x[p]*(1+i.shoalGain*(xe-1)),Y+=I[p]}const ne=P(0,.35,C);d.edge[m]=ne;const re=i.breakRatio*C/2,ve=smoothMin(Y,re,re*.3)*ne,be=Y>T?ve/Y:1;let J=0;for(let p=0;p<s;p++){const F=I[p]*be;d.amp[m*s+p]=F,d.sharp[m*s+p]=F*sharpenAt(F,d.k[m*s+p],C,i.crestSharpen),J+=F}const se=breakAmount(Y*2,C,i);d.breaking[m]=se,V=Math.max(V,J);const ye=V>T?J/V:0;K=Math.max(se*ye,K*Math.exp(-Math.abs(B)/W)),d.foamBed[m]=h?Math.max(K,h.foam*Math.exp(-(((M-h.z)/h.width)**2))):K}for(let m=0;m<l;m++){const M=Math.max(0,m-1),B=Math.min(l-1,m+1),_=a[B]-a[M];d.liftSlope[m]=Math.abs(_)>1e-6?(d.lift[B]-d.lift[M])/_:0}return d}export function breakRow(a,t=.5){for(let e=a.breaking.length-1;e>=0;e--)if(a.depth[e]>0&&a.breaking[e]>=t)return e;return-1}function X(a,t,e){return a<t?t:a>e?e:a}export function smoothMin(a,t,e){if(e<=T)return Math.min(a,t);const n=X(.5+.5*(t-a)/e,0,1);return t*(1-n)+a*n-e*n*(1-n)}function P(a,t,e){if(t<=a)return e>=t?1:0;const n=X((e-a)/(t-a),0,1);return n*n*(3-2*n)}const le=`
uniform float uTime;
uniform vec4 uOmega;
...
`+pe))},n.customProgramCacheKey=()=>t?"ocean-water-1-sky":"ocean-water-1",n}function Ae(){const{rows:a,cols:t}=L,e=a*t;if(!y){y={phase:new Float32Array(e*4),amp:new Float32Array(e*4),waveK:new Float32Array(e*4),shore:new Float32Array(e*4),sharp:new Float32Array(e*4),liftSlope:new Float32Array(e)};for(const o of["phase","amp","waveK","shore","sharp"]){const r=new THREE.BufferAttribute(y[o],4);r.setUsage(THREE.DynamicDrawUsage),k.setAttribute("a"+o.charAt(0).toUpperCase()+o.slice(1),r)}const n=new THREE.BufferAttribute(y.liftSlope,1);n.setUsage(THREE.DynamicDrawUsage),k.setAttribute("aLiftSlope",n)}te()}function te(){const{rows:a,cols:t}=L,e=S.length,n=Math.PI*2;for(let o=0;o<a;o++){const r=o*t*4,i=c.phase[o*e],l=e>1?c.phase[o*e+1]:0,g=e>2?c.phase[o*e+2]:0,s=e>3?c.phase[o*e+3]:0,u=[i-Math.floor(i/n)*n,l-Math.floor(l/n)*n,g-Math.floor(g/n)*n,s-Math.floor(s/n)*n];for(let w=0;w<t;w++){const h=r+w*4;for(let f=0;f<4;f++)y.phase[h+f]=u[f],y.amp[h+f]=f<e?c.amp[o*e+f]:0,y.sharp[h+f]=f<e?c.sharp[o*e+f]:0,y.waveK[h+f]=f<e?c.k[o*e+f]:1;y.shore[h]=c.depth[o],y.shore[h+1]=c.breaking[o],y.shore[h+2]=c.foamBed[o],y.shore[h+3]=c.lift[o],y.liftSlope[r/4+w]=c.liftSlope[o]}}for(const o of["aPhase","aAmp","aWaveK","aShore","aSharp","aLiftSlope"]){const r=k.getAttribute(o);r&&(r.needsUpdate=!0)}Fe()}function Fe(){const{rows:a,cols:t}=L;let e=k.getAttribute("color");e||(e=new THREE.BufferAttribute(new Float32Array(a*t*4),4),e.setUsage(THREE.DynamicDrawUsage),k.setAttribute("color",e),O.vertexColors=!0,O.needsUpdate=!0);const n=e.array;for(let o=0;o<a;o++){const r=c.edge[o];for(let i=0;i<t;i++){const l=(o*t+i)*4;n[l]=1,n[l+1]=1,n[l+2]=1,n[l+3]=r}}e.needsUpdate=!0}export function updateWater(a,t=he){if(!b)return;const e=Math.max(0,Math.min(.25,a));if(R+=e,v&&(v.uTime.value=R),v&&t.lean!=null&&(v.uLeanGain.value=t.lean),v&&z&&t.gloom>0){const o=Math.min(1,t.gloom);v.uDeepColor.value.copy(z.deep).lerp(z.stormDeep,o),v.uShallowColor.value.copy(z.shallow).lerp(z.stormShallow,o);const r=N.water;v.uRoughness.value=r.roughness+(r.stormRoughness-r.roughness)*o}G+=e;const n=1/Math.max(1,profileRate());G>=n&&(G=0,buildProfile(A,R,N,c,t),te()),we()}export function waveSurfaceAt(a,t){if(!c||!A||!S)return null;let e=0,n=1/0;for(let s=0;s<A.length;s++){const u=Math.abs(A[s]-t);u<n&&(n=u,e=s)}const o=S.length;let r=0,i=0,l=0;for(let s=0;s<o;s++){const u=c.k[e*o+s],w=c.amp[e*o+s];if(!(w>0)||!(u>0))continue;const h=Math.max(-.95,Math.min(.95,S[s].kSin/u)),f=Math.sqrt(1-h*h),d=c.phase[e*o+s]+S[s].kSin*a-S[s].omega*R,x=Math.sin(d),W=Math.cos(d),D=c.sharp[e*o+s];r+=w*x-D*Math.cos(2*d);const I=w*W+2*D*Math.sin(2*d);i+=I*u*h,l+=I*u*f}const g=c.lift[e];return l+=c.liftSlope[e],{y:r+g,wave:r,lift:g,slopeX:i,slopeZ:l,row:e,depth:c.depth[e]}}export function resetWater(a=0){const t=Number.isFinite(a)?Math.max(0,a):0;R=t,G=1/0,q=[],H&&H.fill(null),c&&A&&(buildProfile(A,t,N,c),te()),v&&(v.uTime.value=t)}function we(){const a=breakRow(c);if(a<0){H.fill(null);return}const t=S.length,e=Math.PI*2;for(let n=0;n<Math.min(ee,t);n++){const o=c.phase[a*t+n]-S[n].omega*R,r=Math.floor(o/e),i=H[n];if(H[n]=r,i===null||Math.abs(r-i)>2||r>=i)continue;const l=c.amp[a*t+n],g=ge(a),s=X(.25+1.6*(g>T?l/g:0)*c.breaking[a],0,1),u=Math.sin(o*.37+n*2.1)*.6;q.push({strength:s,pan:u,row:a,distance:N.beach.shoreZ-A[a]})}}function ge(a){const t=S.length;let e=0;for(let n=0;n<t;n++)e+=c.amp[a*t+n];return e}export function profileRate(){return Z??(N?N.water.profileHz:E.water.profileHz)}export function setProfileHz(a){if(a==null)return Z=null,profileRate();const t=Number(a);return!Number.isFinite(t)||t<=0?profileRate():(Z=Math.max(1,Math.min(120,t)),G=1/0,Z)}export function consumeBreaks(){const a=q;return q=[],a}export function breakDistance(){if(!c)return 0;const a=breakRow(c);return a<0?0:N.beach.shoreZ-A[a]}export function getWaterMesh(){return b}export function getProfile(){return c}export function getElapsed(){return R}export function disposeWater(){b&&b.parent&&b.parent.remove(b),k&&k.dispose(),O&&O.dispose(),b=null,k=null,O=null,v=null,z=null,y=null,c=null,A=null,L=null,S=null,H=null,q=[],R=0,G=0}export const __test__={GRAVITY:$,SOUNDING_WAVES:ee,clamp:X,smoothstep:P,totalAmpAt:ge,detectBreaks:we,VERTEX_HEAD:le,VERTEX_BODY:ce,VERTEX_POSITION:ue,FRAGMENT_HEAD:fe,FRAGMENT_BODY:de,FRAGMENT_ROUGHNESS:me,FRAGMENT_REFLECT:pe,setElapsed:a=>{R=a},state:()=>({grid:L,profile:c,constants:S,rowZ:A,uniforms:v,attributes:y,cycles:H})};
