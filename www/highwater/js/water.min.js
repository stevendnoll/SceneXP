import{OCEAN_CONFIG as E}from"./config.min.js";const J=9.81,T=1e-6,Q=2,he=Object.freeze({swell:1,surge:0});export function bedHeightAt(a,t=E.beach){return Math.max(-t.maxDepth,(a-t.shoreZ)*t.slope)}export function tideOffset(a,t=E.water){return t.tideRange/2*Math.sin(Math.PI*2*a/t.tidePeriodSeconds)}export function depthAt(a,t,e=E.beach,n=E.water){const o=t-bedHeightAt(a,e);return o<=0?0:Math.max(n.minDepth,o)}export function waveNumberAt(a,t){if(t<=T)return a;const e=Math.tanh(a*t);return a/Math.sqrt(Math.max(e,T))}export function shoalingAt(a,t,e){if(t<=T||a<=T)return 1;const n=a*t,o=n>10?.5:.5*(1+2*n/Math.sinh(2*n)),s=Math.sqrt(J/a*Math.tanh(n));return Math.sqrt(e/Math.max(2*o*s,T))}export function sharpenAt(a,t,e,n=E.water.crestSharpen){if(a<=T||t<=T||e<=T)return 0;const o=Math.tanh(t*e);if(o<=T)return n;const s=a*t/4*(3/(o*o*o)-1/o);return Math.min(n,s)}export function breakAmount(a,t,e=E.water){if(t<=T)return 1;const n=a/t,o=e.breakRatio-e.breakSoftness,s=e.breakRatio+e.breakSoftness;return I(o,s,n)}export function envelopeAt(a,t,e=E.water){const n=t*7.3,o=Math.sin(Math.PI*2*(a+n)/e.setPeriodSeconds),s=Math.sin(Math.PI*2*(a+n)/e.setSubPeriodSeconds);return 1+e.setDepth*o*s}export function rowPositions(a,t=E){const{beach:e,camera:n,water:o}=t,s=new Float32Array(a),i=Math.max(0,Math.min(e.nearRows,a-2)),c=a-i,g=n.z-e.nearZ,r=n.z-e.farZ;for(let u=0;u<i;u++)s[u]=n.z-(g+(e.rowNear-g)*(u/i));for(let u=0;u<c;u++){const w=c>1?u/(c-1):0;s[i+u]=n.z-(e.rowNear+(r-e.rowNear)*Math.pow(w,o.rowBias))}return s}export function halfWidthAt(a,t=E.beach,e=E.camera){return t.baseHalfWidth+Math.max(0,e.z-a)*t.widthPerMetre}export function waveConstants(a){return a.map(t=>{const e=Math.PI*2/t.length,n=Math.sqrt(J/e),o=Math.max(-.95,Math.min(.95,t.dirX));return{k0:e,c0:n,omega:e*n*t.speed,kSin:e*o,amplitude:t.amplitude,steepness:t.steepness}})}export function buildProfile(a,t,e=E,n=null,o=he){const{beach:s,water:i}=e,c=a.length,g=waveConstants(i.waves),r=g.length,u=o.swell>0?o.swell:0,w=tideOffset(t,i)+o.surge,h=o.front||null,f=o.lull||null,m=n||{depth:new Float32Array(c),lift:new Float32Array(c),breaking:new Float32Array(c),foamBed:new Float32Array(c),edge:new Float32Array(c),phase:new Float32Array(c*r),amp:new Float32Array(c*r),sharp:new Float32Array(c*r),k:new Float32Array(c*r)},x=[];for(let p=0;p<r;p++)x.push(envelopeAt(t,p,i));const _=4+40*i.foamPersistence,D=new Float32Array(r),P=new Float32Array(r);let q=0,X=0,ee=a[c-1];for(let p=c-1;p>=0;p--){const z=a[p],te=z-ee;ee=z;let ae=w,K=0;h&&(K=1-I(h.z-h.width,h.z+h.width,z),ae+=h.rise*K);let G=u;if(f){const d=I(f.z-f.width,f.z+f.width,z);G=u+(f.before-u)*d}h&&(G=G+(h.swellBehind-G)*K);let oe=w;if(h){const d=Math.max(0,h.z-z),M=h.body>0?1-I(h.body*.35,h.body,d):1;oe+=h.rise*K*M}m.lift[p]=oe;const C=depthAt(z,ae,s,i);if(m.depth[p]=C,C<=0){m.breaking[p]=0,m.foamBed[p]=0,m.edge[p]=0;for(let d=0;d<r;d++)m.phase[p*r+d]=D[d],m.amp[p*r+d]=0,m.sharp[p*r+d]=0,m.k[p*r+d]=g[d].k0;continue}let B=0;for(let d=0;d<r;d++){const M=g[d],V=waveNumberAt(M.k0,C);m.k[p*r+d]=V;const ie=U(M.kSin/V,-.95,.95),Te=Math.sqrt(1-ie*ie);D[d]+=V*Te*te,m.phase[p*r+d]=D[d];const xe=shoalingAt(V,C,M.c0);P[d]=M.amplitude*G*x[d]*(1+i.shoalGain*(xe-1)),B+=P[d]}const ne=I(0,.35,C);m.edge[p]=ne;const re=i.breakRatio*C/2,ve=smoothMin(B,re,re*.3)*ne,be=B>T?ve/B:1;let j=0;for(let d=0;d<r;d++){const M=P[d]*be;m.amp[p*r+d]=M,m.sharp[p*r+d]=M*sharpenAt(M,m.k[p*r+d],C,i.crestSharpen),j+=M}const se=breakAmount(B*2,C,i);m.breaking[p]=se,X=Math.max(X,j);const ye=X>T?j/X:0;q=Math.max(se*ye,q*Math.exp(-Math.abs(te)/_)),m.foamBed[p]=h?Math.max(q,h.foam*Math.exp(-(((z-h.z)/h.width)**2))):q}return m}export function breakRow(a,t=.5){for(let e=a.breaking.length-1;e>=0;e--)if(a.depth[e]>0&&a.breaking[e]>=t)return e;return-1}function U(a,t,e){return a<t?t:a>e?e:a}export function smoothMin(a,t,e){if(e<=T)return Math.min(a,t);const n=U(.5+.5*(t-a)/e,0,1);return t*(1-n)+a*n-e*n*(1-n)}function I(a,t,e){if(t<=a)return e>=t?1:0;const n=U((e-a)/(t-a),0,1);return n*n*(3-2*n)}const le=`
uniform float uTime;
uniform vec4 uOmega;
...
`;let y=null,N=null,F=null,v=null,O=null,R=null,S=null,l=null,b=null,L=null,A=null,k=0,W=0,Y=[],Z=null,H=null;export function initWater(a,t=E,e={}){R=t;const n=t.water,o=e.mobile?n.mobileScale:1,s=Math.max(8,Math.round(n.rows*o)),i=Math.max(8,Math.round(n.cols*o));return L={rows:s,cols:i},S=waveConstants(n.waves),b=rowPositions(s,t),l=buildProfile(b,0,t),H=new Array(Q).fill(null),F=Ee(s,i,b,t.beach),N=Se(t,e.sky||null),Ae(),y=new THREE.Mesh(F,N),y.name="ocean",y.matrixAutoUpdate=!1,y.updateMatrix(),y.frustumCulled=!1,y.renderOrder=1,a&&a.add(y),y}function Ee(a,t,e,n){const o=new THREE.BufferGeometry,s=a*t,i=new Float32Array(s*3),c=new Float32Array(s*3);for(let w=0;w<a;w++){const h=halfWidthAt(e[w],n);for(let f=0;f<t;f++){const m=t>1?f/(t-1):.5,x=(w*t+f)*3;i[x]=(m*2-1)*h,i[x+1]=0,i[x+2]=e[w],c[x+1]=1}}const g=s>65535?Uint32Array:Uint16Array,r=new g((a-1)*(t-1)*6);let u=0;for(let w=0;w<a-1;w++)for(let h=0;h<t-1;h++){const f=w*t+h,m=f+1,x=f+t,_=x+1;r[u++]=f,r[u++]=m,r[u++]=x,r[u++]=m,r[u++]=_,r[u++]=x}return o.setAttribute("position",new THREE.BufferAttribute(i,3)),o.setAttribute("normal",new THREE.BufferAttribute(c,3)),o.setIndex(new THREE.BufferAttribute(r,1)),o}function Se(a,t=null){const e=a.water,n=new THREE.MeshStandardMaterial({color:16777215,roughness:e.roughness,metalness:0,transparent:!0,side:THREE.DoubleSide});O={deep:new THREE.Color(e.deepColor),shallow:new THREE.Color(e.shallowColor),stormDeep:new THREE.Color(e.stormDeepColor),stormShallow:new THREE.Color(e.stormShallowColor)},v={uTime:{value:0},uOmega:{value:new THREE.Vector4},uSteepness:{value:new THREE.Vector4},uKSin:{value:new THREE.Vector4},uLeanGain:{value:e.leanGain},uDeepColor:{value:new THREE.Color(e.deepColor)},uShallowColor:{value:new THREE.Color(e.shallowColor)},uFoamColor:{value:new THREE.Color(e.foamColor)},uRoughness:{value:e.roughness},uOpacityNear:{value:e.opacityNear},uFoamBreakThreshold:{value:e.foamBreakThreshold},uFoamCrestThreshold:{value:e.foamCrestThreshold},uFoamNoiseScale:{value:e.foamNoiseScale},uFoamDrift:{value:e.foamDriftSpeed},uFoamLag:{value:e.foamLag},uFoamTrail:{value:e.foamTrail},uFoamSheetLag:{value:e.foamSheetLag},uFoamSheetTrail:{value:e.foamSheetTrail},uDeepReference:{value:a.beach.maxDepth*.55}};for(let o=0;o<4;o++){const s=S[o],i=["x","y","z","w"][o];v.uOmega.value[i]=s?s.omega:0,v.uSteepness.value[i]=s?s.steepness:0,v.uKSin.value[i]=s?s.kSin:0}return n.onBeforeCompile=o=>{Object.assign(o.uniforms,v),t&&Object.assign(o.uniforms,t.uniforms),o.vertexShader=o.vertexShader.replace("#include <common>",`#include <common>
...
`+pe))},n.customProgramCacheKey=()=>t?"ocean-water-1-sky":"ocean-water-1",n}function Ae(){const{rows:a,cols:t}=L,e=a*t;if(!A){A={phase:new Float32Array(e*4),amp:new Float32Array(e*4),waveK:new Float32Array(e*4),shore:new Float32Array(e*4),sharp:new Float32Array(e*4)};for(const n of["phase","amp","waveK","shore","sharp"]){const o=new THREE.BufferAttribute(A[n],4);o.setUsage(THREE.DynamicDrawUsage),F.setAttribute("a"+n.charAt(0).toUpperCase()+n.slice(1),o)}}$()}function $(){const{rows:a,cols:t}=L,e=S.length,n=Math.PI*2;for(let o=0;o<a;o++){const s=o*t*4,i=l.phase[o*e],c=e>1?l.phase[o*e+1]:0,g=e>2?l.phase[o*e+2]:0,r=e>3?l.phase[o*e+3]:0,u=[i-Math.floor(i/n)*n,c-Math.floor(c/n)*n,g-Math.floor(g/n)*n,r-Math.floor(r/n)*n];for(let w=0;w<t;w++){const h=s+w*4;for(let f=0;f<4;f++)A.phase[h+f]=u[f],A.amp[h+f]=f<e?l.amp[o*e+f]:0,A.sharp[h+f]=f<e?l.sharp[o*e+f]:0,A.waveK[h+f]=f<e?l.k[o*e+f]:1;A.shore[h]=l.depth[o],A.shore[h+1]=l.breaking[o],A.shore[h+2]=l.foamBed[o],A.shore[h+3]=l.lift[o]}}for(const o of["aPhase","aAmp","aWaveK","aShore","aSharp"]){const s=F.getAttribute(o);s&&(s.needsUpdate=!0)}Me()}function Me(){const{rows:a,cols:t}=L;let e=F.getAttribute("color");e||(e=new THREE.BufferAttribute(new Float32Array(a*t*4),4),e.setUsage(THREE.DynamicDrawUsage),F.setAttribute("color",e),N.vertexColors=!0,N.needsUpdate=!0);const n=e.array;for(let o=0;o<a;o++){const s=l.edge[o];for(let i=0;i<t;i++){const c=(o*t+i)*4;n[c]=1,n[c+1]=1,n[c+2]=1,n[c+3]=s}}e.needsUpdate=!0}export function updateWater(a,t=he){if(!y)return;const e=Math.max(0,Math.min(.25,a));if(k+=e,v&&(v.uTime.value=k),v&&t.lean!=null&&(v.uLeanGain.value=t.lean),v&&O&&t.gloom>0){const o=Math.min(1,t.gloom);v.uDeepColor.value.copy(O.deep).lerp(O.stormDeep,o),v.uShallowColor.value.copy(O.shallow).lerp(O.stormShallow,o);const s=R.water;v.uRoughness.value=s.roughness+(s.stormRoughness-s.roughness)*o}W+=e;const n=1/Math.max(1,profileRate());W>=n&&(W=0,buildProfile(b,k,R,l,t),$()),we()}export function waveSurfaceAt(a,t){if(!l||!b||!S)return null;let e=0,n=1/0;for(let r=0;r<b.length;r++){const u=Math.abs(b[r]-t);u<n&&(n=u,e=r)}const o=S.length;let s=0,i=0,c=0;for(let r=0;r<o;r++){const u=l.k[e*o+r],w=l.amp[e*o+r];if(!(w>0)||!(u>0))continue;const h=Math.max(-.95,Math.min(.95,S[r].kSin/u)),f=Math.sqrt(1-h*h),m=l.phase[e*o+r]+S[r].kSin*a-S[r].omega*k,x=Math.sin(m),_=Math.cos(m),D=l.sharp[e*o+r];s+=w*x-D*Math.cos(2*m);const P=w*_+2*D*Math.sin(2*m);i+=P*u*h,c+=P*u*f}const g=l.lift[e];if(e>0&&e<b.length-1){const r=b[e+1]-b[e-1];Math.abs(r)>1e-6&&(c+=(l.lift[e+1]-l.lift[e-1])/r)}return{y:s+g,wave:s,lift:g,slopeX:i,slopeZ:c,row:e,depth:l.depth[e]}}export function resetWater(){k=0,W=0,Y=[],H&&H.fill(null),l&&b&&(buildProfile(b,0,R,l),$()),v&&(v.uTime.value=0)}function we(){const a=breakRow(l);if(a<0){H.fill(null);return}const t=S.length,e=Math.PI*2;for(let n=0;n<Math.min(Q,t);n++){const o=l.phase[a*t+n]-S[n].omega*k,s=Math.floor(o/e),i=H[n];if(H[n]=s,i===null||Math.abs(s-i)>2||s>=i)continue;const c=l.amp[a*t+n],g=ge(a),r=U(.25+1.6*(g>T?c/g:0)*l.breaking[a],0,1),u=Math.sin(o*.37+n*2.1)*.6;Y.push({strength:r,pan:u,row:a,distance:R.beach.shoreZ-b[a]})}}function ge(a){const t=S.length;let e=0;for(let n=0;n<t;n++)e+=l.amp[a*t+n];return e}export function profileRate(){return Z??(R?R.water.profileHz:E.water.profileHz)}export function setProfileHz(a){if(a==null)return Z=null,profileRate();const t=Number(a);return!Number.isFinite(t)||t<=0?profileRate():(Z=Math.max(1,Math.min(120,t)),W=1/0,Z)}export function consumeBreaks(){const a=Y;return Y=[],a}export function breakDistance(){if(!l)return 0;const a=breakRow(l);return a<0?0:R.beach.shoreZ-b[a]}export function getWaterMesh(){return y}export function getProfile(){return l}export function getElapsed(){return k}export function disposeWater(){y&&y.parent&&y.parent.remove(y),F&&F.dispose(),N&&N.dispose(),y=null,F=null,N=null,v=null,O=null,A=null,l=null,b=null,L=null,S=null,H=null,Y=[],k=0,W=0}export const __test__={GRAVITY:J,SOUNDING_WAVES:Q,clamp:U,smoothstep:I,totalAmpAt:ge,detectBreaks:we,VERTEX_HEAD:le,VERTEX_BODY:ce,VERTEX_POSITION:ue,FRAGMENT_HEAD:fe,FRAGMENT_BODY:de,FRAGMENT_ROUGHNESS:me,FRAGMENT_REFLECT:pe,setElapsed:a=>{k=a},state:()=>({grid:L,profile:l,constants:S,rowZ:b,uniforms:v,attributes:A,cycles:H})};
