...
const XO_CONFIG = {
    ...
    pose: {
         ...
         * `xo-gameplay`'s "no held pose swings an arm out of its own
     ...
     *  game under the lights, and night flatters low-poly geometry. */
    lighting: {
    ...
    /**
     * THE CROWD, IN THE STANDS FROM THE FIRST FRAME (2026-09-14).
     *
     * The stands shipped empty (D8) and read as a practice facility. Now each
     * one is a home crowd: the Mongooses' fans in orange on one side, the
     * Crows' in blue on the other, with a few away sections in each and the
     * odd plain jacket. They jump when their team does something.
     *
     * 500 STILL FILLS THE STANDS. The regulars leave seats empty on purpose,
     * and the perfect game packs every one of them (see `milestones.finale`).
     * The two share one seat grid in stunt.js, so they can never overlap.
     *
     * EACH FAN IS A WHOLE PERSON (same day, second pass): the shared figure
     * the players are built from, with hair, eyes, nose, arms and legs, baked
     * by fans.js into about fifteen instanced pieces for the entire crowd. There
    ...
    crowd: {
        ...
        shirts: {
            0: ['#ff992c', '#e8801f', '#ffb057'],
            1: ['#9bcfff', '#79b2ea', '#c2e2ff'],
            neutral: ['#e9edf3', '#4a5466'],
        },
        skins: ['#f1c7a0', '#d9a276', '#a8704a', '#7a4d2f'],
        /** Hair: a style from fans.js HAIR or 'bald', and a colour. Listed more
         *  than once to weight the draw. */
        hairStyles: ['short', 'short', 'long', 'long', 'buzz', 'bald'],
        hairColours: ['#2b1d14', '#1a1410', '#5a3a22', '#8a5a2e', '#c9a36a', '#8c8c8c'],
        pants: ['#2e3a55', '#23262e', '#4a4f5a', '#5b4a36'],
         ...
         * gets the small one. Lengths are seconds, hops metres; `rate` is hops
         * a second, `stagger` spreads the start across a section, and `ease`
     ...
     * crowd, which was dropped (D8). The crowd that came back sits down the
     * sidelines (see `crowd`), which leaves this strip to the board.
     *
    ...
    milestones: {
        ...
        /**
         * 500, THE PERFECT GAME. The lights go down, fans pack every seat the
         * regulars left empty (see `crowd`), the far stand holds up a card
         * stunt that reads PERFECT and then
        ...
        finale: {
            ...
            confetti: {
                from: 6.0, pieces: 520, size: 0.42,
                palette: ['#ff992c', '#9bcfff', '#f2f5fa', '#ffcf5a', '#e8603c'],
            },
...
deepFreeze(XO_CONFIG);

export {
    XO_CONFIG, FIELD, SIM, UNITS_TO_METRES,
