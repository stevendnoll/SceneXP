 ...
 *
 * THE STANDS ARE A SHELL HERE. The 2D game's crowd did not look right and was
 * dropped (D8), and the empty stands read as a practice facility. The fans
 * came back on 2026-09-14 as instanced meshes in spectacle.js, seated by
 * stunt.js on the risers `standLayout` describes, so this file still builds
 * only the concrete.
 *
...
/**
 * The stands. A raked shell down both sidelines, with the crowd seated on it
 * by spectacle.js (see `crowd` in config.js).
 *
 ...
 * rows read as black voids rather than as structure, and the gaps between the
 * fans should look like empty seats rather than like holes in the world.
 */
/**
 * THE STANDS, AS NUMBERS: where each riser is, so the crowd can sit on them
 * and the finale can put a card stunt in their hands without a copy of these
 * getting out of step with the geometry.
 *
 ...
 * the same strip in the 2D game was dropped (D8). The crowd that came back sits
 * down the sidelines instead.
 *
