import{OCEAN_CONFIG as T}from"./config.min.js";const J=9.81,y=1e-6,Q=2,ie=Object.freeze({swell:1,surge:0});export function bedHeightAt(a,t=T.beach){return Math.max(-t.maxDepth,(a-t.shoreZ)*t.slope)}export function tideOffset(a,t=T.water){return t.tideRange/2*Math.sin(Math.PI*2*a/t.tidePeriodSeconds)}export function depthAt(a,t,e=T.beach,o=T.water){const r=t-bedHeightAt(a,e);return r<=0?0:Math.max(o.minDepth,r)}export function waveNumberAt(a,t){if(t<=y)return a;const e=Math.tanh(a*t);return a/Math.sqrt(Math.max(e,y))}export function shoalingAt(a,t,e){if(t<=y||a<=y)return 1;const o=a*t,r=o>10?.5:.5*(1+2*o/Math.sinh(2*o)),n=Math.sqrt(J/a*Math.tanh(o));return Math.sqrt(e/Math.max(2*r*n,y))}export function sharpenAt(a,t,e,o=T.water.crestSharpen){if(a<=y||t<=y||e<=y)return 0;const r=Math.tanh(t*e);if(r<=y)return o;const n=a*t/4*(3/(r*r*r)-1/r);return Math.min(o,n)}export function breakAmount(a,t,e=T.water){if(t<=y)return 1;const o=a/t,r=e.breakRatio-e.breakSoftness,n=e.breakRatio+e.breakSoftness;return _(r,n,o)}export function envelopeAt(a,t,e=T.water){const o=t*7.3,r=Math.sin(Math.PI*2*(a+o)/e.setPeriodSeconds),n=Math.sin(Math.PI*2*(a+o)/e.setSubPeriodSeconds);return 1+e.setDepth*r*n}export function rowPositions(a,t=T){const{beach:e,camera:o,water:r}=t,n=new Float32Array(a),s=Math.max(0,Math.min(e.nearRows,a-2)),c=a-s,b=o.z-e.nearZ,h=o.z-e.farZ;for(let u=0;u<s;u++)n[u]=o.z-(b+(e.rowNear-b)*(u/s));for(let u=0;u<c;u++){const g=c>1?u/(c-1):0;n[s+u]=o.z-(e.rowNear+(h-e.rowNear)*Math.pow(g,r.rowBias))}return n}export function halfWidthAt(a,t=T.beach,e=T.camera){return t.baseHalfWidth+Math.max(0,e.z-a)*t.widthPerMetre}export function waveConstants(a){return a.map(t=>{const e=Math.PI*2/t.length,o=Math.sqrt(J/e),r=Math.max(-.95,Math.min(.95,t.dirX));return{k0:e,c0:o,omega:e*o*t.speed,kSin:e*r,amplitude:t.amplitude,steepness:t.steepness}})}export function buildProfile(a,t,e=T,o=null,r=ie){const{beach:n,water:s}=e,c=a.length,b=waveConstants(s.waves),h=b.length,u=r.swell>0?r.swell:0,g=tideOffset(t,s)+r.surge,p=r.front||null,d=r.lull||null,i=o||{depth:new Float32Array(c),breaking:new Float32Array(c),foamBed:new Float32Array(c),edge:new Float32Array(c),phase:new Float32Array(c*h),amp:new Float32Array(c*h),sharp:new Float32Array(c*h),k:new Float32Array(c*h)},E=[];for(let m=0;m<h;m++)E.push(envelopeAt(t,m,s));const q=4+40*s.foamPersistence,X=new Float32Array(h),K=new Float32Array(h);let U=0,Y=0,ee=a[c-1];for(let m=c-1;m>=0;m--){const O=a[m],te=O-ee;ee=O;let ae=g,V=0;p&&(V=1-_(p.z-p.width,p.z+p.width,O),ae+=p.rise*V);let I=u;if(d){const l=_(d.z-d.width,d.z+d.width,O);I=u+(d.before-u)*l}p&&(I=I+(p.swellBehind-I)*V);const M=depthAt(O,ae,n,s);if(i.depth[m]=M,M<=0){i.breaking[m]=0,i.foamBed[m]=0,i.edge[m]=0;for(let l=0;l<h;l++)i.phase[m*h+l]=X[l],i.amp[m*h+l]=0,i.sharp[m*h+l]=0,i.k[m*h+l]=b[l].k0;continue}let L=0;for(let l=0;l<h;l++){const A=b[l],Z=waveNumberAt(A.k0,M);i.k[m*h+l]=Z;const se=W(A.kSin/Z,-.95,.95),ye=Math.sqrt(1-se*se);X[l]+=Z*ye*te,i.phase[m*h+l]=X[l];const Te=shoalingAt(Z,M,A.c0);K[l]=A.amplitude*I*E[l]*(1+s.shoalGain*(Te-1)),L+=K[l]}const oe=_(0,.35,M);i.edge[m]=oe;const re=s.breakRatio*M/2,ge=smoothMin(L,re,re*.3)*oe,ve=L>y?ge/L:1;let j=0;for(let l=0;l<h;l++){const A=K[l]*ve;i.amp[m*h+l]=A,i.sharp[m*h+l]=A*sharpenAt(A,i.k[m*h+l],M,s.crestSharpen),j+=A}const ne=breakAmount(L*2,M,s);i.breaking[m]=ne,Y=Math.max(Y,j);const be=Y>y?j/Y:0;U=Math.max(ne*be,U*Math.exp(-Math.abs(te)/q)),i.foamBed[m]=p?Math.max(U,p.foam*Math.exp(-(((O-p.z)/p.width)**2))):U}return i}export function breakRow(a,t=.5){for(let e=a.breaking.length-1;e>=0;e--)if(a.depth[e]>0&&a.breaking[e]>=t)return e;return-1}function W(a,t,e){return a<t?t:a>e?e:a}export function smoothMin(a,t,e){if(e<=y)return Math.min(a,t);const o=W(.5+.5*(t-a)/e,0,1);return t*(1-o)+a*o-e*o*(1-o)}function _(a,t,e){if(t<=a)return e>=t?1:0;const o=W((e-a)/(t-a),0,1);return o*o*(3-2*o)}const he=`
uniform float uTime;
uniform vec4 uOmega;
...
`;let v=null,N=null,F=null,w=null,z=null,C=null,R=null,f=null,S=null,D=null,x=null,k=0,P=0,B=[],G=null,H=null;export function initWater(a,t=T,e={}){C=t;const o=t.water,r=e.mobile?o.mobileScale:1,n=Math.max(8,Math.round(o.rows*r)),s=Math.max(8,Math.round(o.cols*r));return D={rows:n,cols:s},R=waveConstants(o.waves),S=rowPositions(n,t),f=buildProfile(S,0,t),H=new Array(Q).fill(null),F=xe(n,s,S,t.beach),N=Ee(t,e.sky||null),Se(),v=new THREE.Mesh(F,N),v.name="ocean",v.matrixAutoUpdate=!1,v.updateMatrix(),v.frustumCulled=!1,v.renderOrder=1,a&&a.add(v),v}function xe(a,t,e,o){const r=new THREE.BufferGeometry,n=a*t,s=new Float32Array(n*3),c=new Float32Array(n*3);for(let g=0;g<a;g++){const p=halfWidthAt(e[g],o);for(let d=0;d<t;d++){const i=t>1?d/(t-1):.5,E=(g*t+d)*3;s[E]=(i*2-1)*p,s[E+1]=0,s[E+2]=e[g],c[E+1]=1}}const b=n>65535?Uint32Array:Uint16Array,h=new b((a-1)*(t-1)*6);let u=0;for(let g=0;g<a-1;g++)for(let p=0;p<t-1;p++){const d=g*t+p,i=d+1,E=d+t,q=E+1;h[u++]=d,h[u++]=i,h[u++]=E,h[u++]=i,h[u++]=q,h[u++]=E}return r.setAttribute("position",new THREE.BufferAttribute(s,3)),r.setAttribute("normal",new THREE.BufferAttribute(c,3)),r.setIndex(new THREE.BufferAttribute(h,1)),r}function Ee(a,t=null){const e=a.water,o=new THREE.MeshStandardMaterial({color:16777215,roughness:.08,metalness:0,transparent:!0,side:THREE.DoubleSide});z={deep:new THREE.Color(e.deepColor),shallow:new THREE.Color(e.shallowColor),stormDeep:new THREE.Color(e.stormDeepColor),stormShallow:new THREE.Color(e.stormShallowColor)},w={uTime:{value:0},uOmega:{value:new THREE.Vector4},uSteepness:{value:new THREE.Vector4},uKSin:{value:new THREE.Vector4},uLeanGain:{value:e.leanGain},uDeepColor:{value:new THREE.Color(e.deepColor)},uShallowColor:{value:new THREE.Color(e.shallowColor)},uFoamColor:{value:new THREE.Color(e.foamColor)},uOpacityNear:{value:e.opacityNear},uFoamBreakThreshold:{value:e.foamBreakThreshold},uFoamCrestThreshold:{value:e.foamCrestThreshold},uFoamNoiseScale:{value:e.foamNoiseScale},uFoamDrift:{value:e.foamDriftSpeed},uFoamLag:{value:e.foamLag},uFoamTrail:{value:e.foamTrail},uFoamSheetLag:{value:e.foamSheetLag},uFoamSheetTrail:{value:e.foamSheetTrail},uDeepReference:{value:a.beach.maxDepth*.55}};for(let r=0;r<4;r++){const n=R[r],s=["x","y","z","w"][r];w.uOmega.value[s]=n?n.omega:0,w.uSteepness.value[s]=n?n.steepness:0,w.uKSin.value[s]=n?n.kSin:0}return o.onBeforeCompile=r=>{Object.assign(r.uniforms,w),t&&Object.assign(r.uniforms,t.uniforms),r.vertexShader=r.vertexShader.replace("#include <common>",`#include <common>
`+he).replace("#include <beginnormal_vertex>",le).replace("#include <begin_vertex>",ce),r.fragmentShader=r.fragmentShader.replace("#include <common>",`#include <common>
`+ue+(t?t.uniformGlsl+t.glsl:"")).replace("#include <roughnessmap_fragment>",de).replace("#include <map_fragment>",`#include <map_fragment>
`+fe),t&&(r.fragmentShader=r.fragmentShader.replace("#include <opaque_fragment>",`#include <opaque_fragment>
`+me))},o.customProgramCacheKey=()=>t?"ocean-water-1-sky":"ocean-water-1",o}function Se(){const{rows:a,cols:t}=D,e=a*t;if(!x){x={phase:new Float32Array(e*4),amp:new Float32Array(e*4),waveK:new Float32Array(e*4),shore:new Float32Array(e*4),sharp:new Float32Array(e*4)};for(const o of["phase","amp","waveK","shore","sharp"]){const r=new THREE.BufferAttribute(x[o],4);r.setUsage(THREE.DynamicDrawUsage),F.setAttribute("a"+o.charAt(0).toUpperCase()+o.slice(1),r)}}$()}function $(){const{rows:a,cols:t}=D,e=R.length,o=Math.PI*2,r=tideOffset(k,C.water);for(let n=0;n<a;n++){const s=n*t*4,c=f.phase[n*e],b=e>1?f.phase[n*e+1]:0,h=e>2?f.phase[n*e+2]:0,u=e>3?f.phase[n*e+3]:0,g=[c-Math.floor(c/o)*o,b-Math.floor(b/o)*o,h-Math.floor(h/o)*o,u-Math.floor(u/o)*o];for(let p=0;p<t;p++){const d=s+p*4;for(let i=0;i<4;i++)x.phase[d+i]=g[i],x.amp[d+i]=i<e?f.amp[n*e+i]:0,x.sharp[d+i]=i<e?f.sharp[n*e+i]:0,x.waveK[d+i]=i<e?f.k[n*e+i]:1;x.shore[d]=f.depth[n],x.shore[d+1]=f.breaking[n],x.shore[d+2]=f.foamBed[n],x.shore[d+3]=r}}for(const n of["aPhase","aAmp","aWaveK","aShore","aSharp"]){const s=F.getAttribute(n);s&&(s.needsUpdate=!0)}Ae()}function Ae(){const{rows:a,cols:t}=D;let e=F.getAttribute("color");e||(e=new THREE.BufferAttribute(new Float32Array(a*t*4),4),e.setUsage(THREE.DynamicDrawUsage),F.setAttribute("color",e),N.vertexColors=!0,N.needsUpdate=!0);const o=e.array;for(let r=0;r<a;r++){const n=f.edge[r];for(let s=0;s<t;s++){const c=(r*t+s)*4;o[c]=1,o[c+1]=1,o[c+2]=1,o[c+3]=n}}e.needsUpdate=!0}export function updateWater(a,t=ie){if(!v)return;const e=Math.max(0,Math.min(.25,a));if(k+=e,w&&(w.uTime.value=k),w&&t.lean!=null&&(w.uLeanGain.value=t.lean),w&&z&&t.gloom>0){const r=Math.min(1,t.gloom);w.uDeepColor.value.copy(z.deep).lerp(z.stormDeep,r),w.uShallowColor.value.copy(z.shallow).lerp(z.stormShallow,r)}P+=e;const o=1/Math.max(1,profileRate());P>=o&&(P=0,buildProfile(S,k,C,f,t),$()),pe()}export function resetWater(){k=0,P=0,B=[],H&&H.fill(null),f&&S&&(buildProfile(S,0,C,f),$()),w&&(w.uTime.value=0)}function pe(){const a=breakRow(f);if(a<0){H.fill(null);return}const t=R.length,e=Math.PI*2;for(let o=0;o<Math.min(Q,t);o++){const r=f.phase[a*t+o]-R[o].omega*k,n=Math.floor(r/e),s=H[o];if(H[o]=n,s===null||Math.abs(n-s)>2||n>=s)continue;const c=f.amp[a*t+o],b=we(a),h=W(.25+1.6*(b>y?c/b:0)*f.breaking[a],0,1),u=Math.sin(r*.37+o*2.1)*.6;B.push({strength:h,pan:u,row:a,distance:C.beach.shoreZ-S[a]})}}function we(a){const t=R.length;let e=0;for(let o=0;o<t;o++)e+=f.amp[a*t+o];return e}export function profileRate(){return G??(C?C.water.profileHz:T.water.profileHz)}export function setProfileHz(a){if(a==null)return G=null,profileRate();const t=Number(a);return!Number.isFinite(t)||t<=0?profileRate():(G=Math.max(1,Math.min(120,t)),P=1/0,G)}export function consumeBreaks(){const a=B;return B=[],a}export function breakDistance(){if(!f)return 0;const a=breakRow(f);return a<0?0:C.beach.shoreZ-S[a]}export function getWaterMesh(){return v}export function getProfile(){return f}export function getElapsed(){return k}export function disposeWater(){v&&v.parent&&v.parent.remove(v),F&&F.dispose(),N&&N.dispose(),v=null,F=null,N=null,w=null,z=null,x=null,f=null,S=null,D=null,R=null,H=null,B=[],k=0,P=0}export const __test__={GRAVITY:J,SOUNDING_WAVES:Q,clamp:W,smoothstep:_,totalAmpAt:we,detectBreaks:pe,VERTEX_HEAD:he,VERTEX_BODY:le,VERTEX_POSITION:ce,FRAGMENT_HEAD:ue,FRAGMENT_BODY:fe,FRAGMENT_ROUGHNESS:de,FRAGMENT_REFLECT:me,setElapsed:a=>{k=a},state:()=>({grid:D,profile:f,constants:R,rowZ:S,uniforms:w,attributes:x,cycles:H})};
