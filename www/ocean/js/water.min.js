import{OCEAN_CONFIG as x}from"./config.min.js";const K=9.81,y=1e-6,V=2,re=Object.freeze({swell:1,surge:0});export function bedHeightAt(a,t=x.beach){return Math.max(-t.maxDepth,(a-t.shoreZ)*t.slope)}export function tideOffset(a,t=x.water){return t.tideRange/2*Math.sin(Math.PI*2*a/t.tidePeriodSeconds)}export function depthAt(a,t,e=x.beach,o=x.water){const r=t-bedHeightAt(a,e);return r<=0?0:Math.max(o.minDepth,r)}export function waveNumberAt(a,t){if(t<=y)return a;const e=Math.tanh(a*t);return a/Math.sqrt(Math.max(e,y))}export function shoalingAt(a,t,e){if(t<=y||a<=y)return 1;const o=a*t,r=o>10?.5:.5*(1+2*o/Math.sinh(2*o)),n=Math.sqrt(K/a*Math.tanh(o));return Math.sqrt(e/Math.max(2*r*n,y))}export function sharpenAt(a,t,e,o=x.water.crestSharpen){if(a<=y||t<=y||e<=y)return 0;const r=Math.tanh(t*e);if(r<=y)return o;const n=a*t/4*(3/(r*r*r)-1/r);return Math.min(o,n)}export function breakAmount(a,t,e=x.water){if(t<=y)return 1;const o=a/t,r=e.breakRatio-e.breakSoftness,n=e.breakRatio+e.breakSoftness;return Z(r,n,o)}export function envelopeAt(a,t,e=x.water){const o=t*7.3,r=Math.sin(Math.PI*2*(a+o)/e.setPeriodSeconds),n=Math.sin(Math.PI*2*(a+o)/e.setSubPeriodSeconds);return 1+e.setDepth*r*n}export function rowPositions(a,t=x){const{beach:e,camera:o,water:r}=t,n=new Float32Array(a),s=Math.max(0,Math.min(e.nearRows,a-2)),l=a-s,b=o.z-e.nearZ,h=o.z-e.farZ;for(let u=0;u<s;u++)n[u]=o.z-(b+(e.rowNear-b)*(u/s));for(let u=0;u<l;u++){const g=l>1?u/(l-1):0;n[s+u]=o.z-(e.rowNear+(h-e.rowNear)*Math.pow(g,r.rowBias))}return n}export function halfWidthAt(a,t=x.beach,e=x.camera){return t.baseHalfWidth+Math.max(0,e.z-a)*t.widthPerMetre}export function waveConstants(a){return a.map(t=>{const e=Math.PI*2/t.length,o=Math.sqrt(K/e),r=Math.max(-.95,Math.min(.95,t.dirX));return{k0:e,c0:o,omega:e*o*t.speed,kSin:e*r,amplitude:t.amplitude,steepness:t.steepness}})}export function buildProfile(a,t,e=x,o=null,r=re){const{beach:n,water:s}=e,l=a.length,b=waveConstants(s.waves),h=b.length,u=r.swell>0?r.swell:0,g=tideOffset(t,s)+r.surge,m=r.front||null,i=o||{depth:new Float32Array(l),breaking:new Float32Array(l),foamBed:new Float32Array(l),edge:new Float32Array(l),phase:new Float32Array(l*h),amp:new Float32Array(l*h),sharp:new Float32Array(l*h),k:new Float32Array(l*h)},p=[];for(let d=0;d<h;d++)p.push(envelopeAt(t,d,s));const E=4+40*s.foamPersistence,z=new Float32Array(h),q=new Float32Array(h);let B=0,G=0,J=a[l-1];for(let d=l-1;d>=0;d--){const P=a[d],Q=P-J;J=P;let $=g,X=0;m&&(X=1-Z(m.z-m.width,m.z+m.width,P),$+=m.rise*X);const me=m?u+(m.swellBehind-u)*X:u,M=depthAt(P,$,n,s);if(i.depth[d]=M,M<=0){i.breaking[d]=0,i.foamBed[d]=0,i.edge[d]=0;for(let c=0;c<h;c++)i.phase[d*h+c]=z[c],i.amp[d*h+c]=0,i.sharp[d*h+c]=0,i.k[d*h+c]=b[c].k0;continue}let I=0;for(let c=0;c<h;c++){const A=b[c],U=waveNumberAt(A.k0,M);i.k[d*h+c]=U;const oe=L(A.kSin/U,-.95,.95),ve=Math.sqrt(1-oe*oe);z[c]+=U*ve*Q,i.phase[d*h+c]=z[c];const be=shoalingAt(U,M,A.c0);q[c]=A.amplitude*me*p[c]*(1+s.shoalGain*(be-1)),I+=q[c]}const ee=Z(0,.35,M);i.edge[d]=ee;const te=s.breakRatio*M/2,pe=smoothMin(I,te,te*.3)*ee,we=I>y?pe/I:1;let Y=0;for(let c=0;c<h;c++){const A=q[c]*we;i.amp[d*h+c]=A,i.sharp[d*h+c]=A*sharpenAt(A,i.k[d*h+c],M,s.crestSharpen),Y+=A}const ae=breakAmount(I*2,M,s);i.breaking[d]=ae,G=Math.max(G,Y);const ge=G>y?Y/G:0;B=Math.max(ae*ge,B*Math.exp(-Math.abs(Q)/E)),i.foamBed[d]=m?Math.max(B,m.foam*Math.exp(-(((P-m.z)/m.width)**2))):B}return i}export function breakRow(a,t=.5){for(let e=a.breaking.length-1;e>=0;e--)if(a.depth[e]>0&&a.breaking[e]>=t)return e;return-1}function L(a,t,e){return a<t?t:a>e?e:a}export function smoothMin(a,t,e){if(e<=y)return Math.min(a,t);const o=L(.5+.5*(t-a)/e,0,1);return t*(1-o)+a*o-e*o*(1-o)}function Z(a,t,e){if(t<=a)return e>=t?1:0;const o=L((e-a)/(t-a),0,1);return o*o*(3-2*o)}const ne=`
uniform float uTime;
uniform vec4 uOmega;
...
`;let v=null,H=null,F=null,w=null,N=null,O=null,C=null,f=null,S=null,D=null,T=null,k=0,W=0,_=[],R=null;export function initWater(a,t=x,e={}){O=t;const o=t.water,r=e.mobile?o.mobileScale:1,n=Math.max(8,Math.round(o.rows*r)),s=Math.max(8,Math.round(o.cols*r));return D={rows:n,cols:s},C=waveConstants(o.waves),S=rowPositions(n,t),f=buildProfile(S,0,t),R=new Array(V).fill(null),F=ye(n,s,S,t.beach),H=Te(t,e.sky||null),xe(),v=new THREE.Mesh(F,H),v.name="ocean",v.matrixAutoUpdate=!1,v.updateMatrix(),v.frustumCulled=!1,v.renderOrder=1,a&&a.add(v),v}function ye(a,t,e,o){const r=new THREE.BufferGeometry,n=a*t,s=new Float32Array(n*3),l=new Float32Array(n*3);for(let g=0;g<a;g++){const m=halfWidthAt(e[g],o);for(let i=0;i<t;i++){const p=t>1?i/(t-1):.5,E=(g*t+i)*3;s[E]=(p*2-1)*m,s[E+1]=0,s[E+2]=e[g],l[E+1]=1}}const b=n>65535?Uint32Array:Uint16Array,h=new b((a-1)*(t-1)*6);let u=0;for(let g=0;g<a-1;g++)for(let m=0;m<t-1;m++){const i=g*t+m,p=i+1,E=i+t,z=E+1;h[u++]=i,h[u++]=p,h[u++]=E,h[u++]=p,h[u++]=z,h[u++]=E}return r.setAttribute("position",new THREE.BufferAttribute(s,3)),r.setAttribute("normal",new THREE.BufferAttribute(l,3)),r.setIndex(new THREE.BufferAttribute(h,1)),r}function Te(a,t=null){const e=a.water,o=new THREE.MeshStandardMaterial({color:16777215,roughness:.08,metalness:0,transparent:!0,side:THREE.DoubleSide});N={deep:new THREE.Color(e.deepColor),shallow:new THREE.Color(e.shallowColor),stormDeep:new THREE.Color(e.stormDeepColor),stormShallow:new THREE.Color(e.stormShallowColor)},w={uTime:{value:0},uOmega:{value:new THREE.Vector4},uSteepness:{value:new THREE.Vector4},uKSin:{value:new THREE.Vector4},uLeanGain:{value:e.leanGain},uDeepColor:{value:new THREE.Color(e.deepColor)},uShallowColor:{value:new THREE.Color(e.shallowColor)},uFoamColor:{value:new THREE.Color(e.foamColor)},uOpacityNear:{value:e.opacityNear},uFoamBreakThreshold:{value:e.foamBreakThreshold},uFoamCrestThreshold:{value:e.foamCrestThreshold},uFoamNoiseScale:{value:e.foamNoiseScale},uFoamDrift:{value:e.foamDriftSpeed},uFoamLag:{value:e.foamLag},uFoamTrail:{value:e.foamTrail},uFoamSheetLag:{value:e.foamSheetLag},uFoamSheetTrail:{value:e.foamSheetTrail},uDeepReference:{value:a.beach.maxDepth*.55}};for(let r=0;r<4;r++){const n=C[r],s=["x","y","z","w"][r];w.uOmega.value[s]=n?n.omega:0,w.uSteepness.value[s]=n?n.steepness:0,w.uKSin.value[s]=n?n.kSin:0}return o.onBeforeCompile=r=>{Object.assign(r.uniforms,w),t&&Object.assign(r.uniforms,t.uniforms),r.vertexShader=r.vertexShader.replace("#include <common>",`#include <common>
`+ne).replace("#include <beginnormal_vertex>",se).replace("#include <begin_vertex>",ie),r.fragmentShader=r.fragmentShader.replace("#include <common>",`#include <common>
`+he+(t?t.uniformGlsl+t.glsl:"")).replace("#include <roughnessmap_fragment>",ce).replace("#include <map_fragment>",`#include <map_fragment>
`+le),t&&(r.fragmentShader=r.fragmentShader.replace("#include <opaque_fragment>",`#include <opaque_fragment>
`+ue))},o.customProgramCacheKey=()=>t?"ocean-water-1-sky":"ocean-water-1",o}function xe(){const{rows:a,cols:t}=D,e=a*t;if(!T){T={phase:new Float32Array(e*4),amp:new Float32Array(e*4),waveK:new Float32Array(e*4),shore:new Float32Array(e*4),sharp:new Float32Array(e*4)};for(const o of["phase","amp","waveK","shore","sharp"]){const r=new THREE.BufferAttribute(T[o],4);r.setUsage(THREE.DynamicDrawUsage),F.setAttribute("a"+o.charAt(0).toUpperCase()+o.slice(1),r)}}j()}function j(){const{rows:a,cols:t}=D,e=C.length,o=Math.PI*2,r=tideOffset(k,O.water);for(let n=0;n<a;n++){const s=n*t*4,l=f.phase[n*e],b=e>1?f.phase[n*e+1]:0,h=e>2?f.phase[n*e+2]:0,u=e>3?f.phase[n*e+3]:0,g=[l-Math.floor(l/o)*o,b-Math.floor(b/o)*o,h-Math.floor(h/o)*o,u-Math.floor(u/o)*o];for(let m=0;m<t;m++){const i=s+m*4;for(let p=0;p<4;p++)T.phase[i+p]=g[p],T.amp[i+p]=p<e?f.amp[n*e+p]:0,T.sharp[i+p]=p<e?f.sharp[n*e+p]:0,T.waveK[i+p]=p<e?f.k[n*e+p]:1;T.shore[i]=f.depth[n],T.shore[i+1]=f.breaking[n],T.shore[i+2]=f.foamBed[n],T.shore[i+3]=r}}for(const n of["aPhase","aAmp","aWaveK","aShore","aSharp"]){const s=F.getAttribute(n);s&&(s.needsUpdate=!0)}Ee()}function Ee(){const{rows:a,cols:t}=D;let e=F.getAttribute("color");e||(e=new THREE.BufferAttribute(new Float32Array(a*t*4),4),e.setUsage(THREE.DynamicDrawUsage),F.setAttribute("color",e),H.vertexColors=!0,H.needsUpdate=!0);const o=e.array;for(let r=0;r<a;r++){const n=f.edge[r];for(let s=0;s<t;s++){const l=(r*t+s)*4;o[l]=1,o[l+1]=1,o[l+2]=1,o[l+3]=n}}e.needsUpdate=!0}export function updateWater(a,t=re){if(!v)return;const e=Math.max(0,Math.min(.25,a));if(k+=e,w&&(w.uTime.value=k),w&&t.lean!=null&&(w.uLeanGain.value=t.lean),w&&N&&t.gloom>0){const r=Math.min(1,t.gloom);w.uDeepColor.value.copy(N.deep).lerp(N.stormDeep,r),w.uShallowColor.value.copy(N.shallow).lerp(N.stormShallow,r)}W+=e;const o=1/Math.max(1,O.water.profileHz);W>=o&&(W=0,buildProfile(S,k,O,f,t),j()),fe()}export function resetWater(){k=0,W=0,_=[],R&&R.fill(null),f&&S&&(buildProfile(S,0,O,f),j()),w&&(w.uTime.value=0)}function fe(){const a=breakRow(f);if(a<0){R.fill(null);return}const t=C.length,e=Math.PI*2;for(let o=0;o<Math.min(V,t);o++){const r=f.phase[a*t+o]-C[o].omega*k,n=Math.floor(r/e),s=R[o];if(R[o]=n,s===null||Math.abs(n-s)>2||n>=s)continue;const l=f.amp[a*t+o],b=de(a),h=L(.25+1.6*(b>y?l/b:0)*f.breaking[a],0,1),u=Math.sin(r*.37+o*2.1)*.6;_.push({strength:h,pan:u,row:a,distance:O.beach.shoreZ-S[a]})}}function de(a){const t=C.length;let e=0;for(let o=0;o<t;o++)e+=f.amp[a*t+o];return e}export function consumeBreaks(){const a=_;return _=[],a}export function breakDistance(){if(!f)return 0;const a=breakRow(f);return a<0?0:O.beach.shoreZ-S[a]}export function getWaterMesh(){return v}export function getProfile(){return f}export function getElapsed(){return k}export function disposeWater(){v&&v.parent&&v.parent.remove(v),F&&F.dispose(),H&&H.dispose(),v=null,F=null,H=null,w=null,N=null,T=null,f=null,S=null,D=null,C=null,R=null,_=[],k=0,W=0}export const __test__={GRAVITY:K,SOUNDING_WAVES:V,clamp:L,smoothstep:Z,totalAmpAt:de,detectBreaks:fe,VERTEX_HEAD:ne,VERTEX_BODY:se,VERTEX_POSITION:ie,FRAGMENT_HEAD:he,FRAGMENT_BODY:le,FRAGMENT_ROUGHNESS:ce,FRAGMENT_REFLECT:ue,setElapsed:a=>{k=a},state:()=>({grid:D,profile:f,constants:C,rowZ:S,uniforms:w,attributes:T,cycles:R})};
