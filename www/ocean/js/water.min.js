import{OCEAN_CONFIG as T}from"./config.min.js";const U=9.81,g=1e-6,_=2;export function bedHeightAt(a,e=T.beach){return Math.max(-e.maxDepth,(a-e.shoreZ)*e.slope)}export function tideOffset(a,e=T.water){return e.tideRange/2*Math.sin(Math.PI*2*a/e.tidePeriodSeconds)}export function depthAt(a,e,t=T.beach,o=T.water){const r=e-bedHeightAt(a,t);return r<=0?0:Math.max(o.minDepth,r)}export function waveNumberAt(a,e){if(e<=g)return a;const t=Math.tanh(a*e);return a/Math.sqrt(Math.max(t,g))}export function shoalingAt(a,e,t){if(e<=g||a<=g)return 1;const o=a*e,r=o>10?.5:.5*(1+2*o/Math.sinh(2*o)),n=Math.sqrt(U/a*Math.tanh(o));return Math.sqrt(t/Math.max(2*r*n,g))}export function sharpenAt(a,e,t,o=T.water.crestSharpen){if(a<=g||e<=g||t<=g)return 0;const r=Math.tanh(e*t);if(r<=g)return o;const n=a*e/4*(3/(r*r*r)-1/r);return Math.min(o,n)}export function breakAmount(a,e,t=T.water){if(e<=g)return 1;const o=a/e,r=t.breakRatio-t.breakSoftness,n=t.breakRatio+t.breakSoftness;return X(r,n,o)}export function envelopeAt(a,e,t=T.water){const o=e*7.3,r=Math.sin(Math.PI*2*(a+o)/t.setPeriodSeconds),n=Math.sin(Math.PI*2*(a+o)/t.setSubPeriodSeconds);return 1+t.setDepth*r*n}export function rowPositions(a,e=T){const{beach:t,camera:o,water:r}=e,n=new Float32Array(a),i=Math.max(0,Math.min(t.nearRows,a-2)),m=a-i,h=o.z-t.nearZ,p=o.z-t.farZ;for(let s=0;s<i;s++)n[s]=o.z-(h+(t.rowNear-h)*(s/i));for(let s=0;s<m;s++){const w=m>1?s/(m-1):0;n[i+s]=o.z-(t.rowNear+(p-t.rowNear)*Math.pow(w,r.rowBias))}return n}export function halfWidthAt(a,e=T.beach,t=T.camera){return e.baseHalfWidth+Math.max(0,t.z-a)*e.widthPerMetre}export function waveConstants(a){return a.map(e=>{const t=Math.PI*2/e.length,o=Math.sqrt(U/t),r=Math.max(-.95,Math.min(.95,e.dirX));return{k0:t,c0:o,omega:t*o*e.speed,kSin:t*r,amplitude:e.amplitude,steepness:e.steepness}})}export function buildProfile(a,e,t=T,o=null){const{beach:r,water:n}=t,i=a.length,m=waveConstants(n.waves),h=m.length,p=tideOffset(e,n),s=o||{depth:new Float32Array(i),breaking:new Float32Array(i),foamBed:new Float32Array(i),edge:new Float32Array(i),phase:new Float32Array(i*h),amp:new Float32Array(i*h),sharp:new Float32Array(i*h),k:new Float32Array(i*h)},w=[];for(let c=0;c<h;c++)w.push(envelopeAt(e,c,n));const y=4+40*n.foamPersistence,d=new Float32Array(h),f=new Float32Array(h);let x=0,P=0,Y=a[i-1];for(let c=i-1;c>=0;c--){const G=a[c],Z=G-Y;Y=G;const k=depthAt(G,p,r,n);if(s.depth[c]=k,k<=0){s.breaking[c]=0,s.foamBed[c]=0,s.edge[c]=0;for(let l=0;l<h;l++)s.phase[c*h+l]=d[l],s.amp[c*h+l]=0,s.sharp[c*h+l]=0,s.k[c*h+l]=m[l].k0;continue}let z=0;for(let l=0;l<h;l++){const S=m[l],I=waveNumberAt(S.k0,k);s.k[c*h+l]=I;const j=C(S.kSin/I,-.95,.95),le=Math.sqrt(1-j*j);d[l]+=I*le*Z,s.phase[c*h+l]=d[l];const ce=shoalingAt(I,k,S.c0);f[l]=S.amplitude*w[l]*(1+n.shoalGain*(ce-1)),z+=f[l]}const q=X(0,.35,k);s.edge[c]=q;const K=n.breakRatio*k/2,se=smoothMin(z,K,K*.3)*q,ie=z>g?se/z:1;let W=0;for(let l=0;l<h;l++){const S=f[l]*ie;s.amp[c*h+l]=S,s.sharp[c*h+l]=S*sharpenAt(S,s.k[c*h+l],k,n.crestSharpen),W+=S}const V=breakAmount(z*2,k,n);s.breaking[c]=V,P=Math.max(P,W);const he=P>g?W/P:0;x=Math.max(V*he,x*Math.exp(-Math.abs(Z)/y)),s.foamBed[c]=x}return s}export function breakRow(a,e=.5){for(let t=a.breaking.length-1;t>=0;t--)if(a.depth[t]>0&&a.breaking[t]>=e)return t;return-1}function C(a,e,t){return a<e?e:a>t?t:a}export function smoothMin(a,e,t){if(t<=g)return Math.min(a,e);const o=C(.5+.5*(e-a)/t,0,1);return e*(1-o)+a*o-t*o*(1-o)}function X(a,e,t){if(e<=a)return t>=e?1:0;const o=C((t-a)/(e-a),0,1);return o*o*(3-2*o)}const J=`
uniform float uTime;
uniform vec4 uOmega;
...
`;let v=null,R=null,E=null,A=null,D=null,F=null,u=null,M=null,N=null,b=null,H=0,L=0,B=[],O=null;export function initWater(a,e=T,t={}){D=e;const o=e.water,r=t.mobile?o.mobileScale:1,n=Math.max(8,Math.round(o.rows*r)),i=Math.max(8,Math.round(o.cols*r));return N={rows:n,cols:i},F=waveConstants(o.waves),M=rowPositions(n,e),u=buildProfile(M,0,e),O=new Array(_).fill(null),E=ue(n,i,M,e.beach),R=fe(e),de(),v=new THREE.Mesh(E,R),v.name="ocean",v.matrixAutoUpdate=!1,v.updateMatrix(),v.frustumCulled=!1,v.renderOrder=1,a&&a.add(v),v}function ue(a,e,t,o){const r=new THREE.BufferGeometry,n=a*e,i=new Float32Array(n*3),m=new Float32Array(n*3);for(let w=0;w<a;w++){const y=halfWidthAt(t[w],o);for(let d=0;d<e;d++){const f=e>1?d/(e-1):.5,x=(w*e+d)*3;i[x]=(f*2-1)*y,i[x+1]=0,i[x+2]=t[w],m[x+1]=1}}const h=n>65535?Uint32Array:Uint16Array,p=new h((a-1)*(e-1)*6);let s=0;for(let w=0;w<a-1;w++)for(let y=0;y<e-1;y++){const d=w*e+y,f=d+1,x=d+e,P=x+1;p[s++]=d,p[s++]=f,p[s++]=x,p[s++]=f,p[s++]=P,p[s++]=x}return r.setAttribute("position",new THREE.BufferAttribute(i,3)),r.setAttribute("normal",new THREE.BufferAttribute(m,3)),r.setIndex(new THREE.BufferAttribute(p,1)),r}function fe(a){const e=a.water,t=new THREE.MeshStandardMaterial({color:16777215,roughness:.08,metalness:0,transparent:!0,side:THREE.DoubleSide});A={uTime:{value:0},uOmega:{value:new THREE.Vector4},uSteepness:{value:new THREE.Vector4},uKSin:{value:new THREE.Vector4},uLeanGain:{value:e.leanGain},uDeepColor:{value:new THREE.Color(e.deepColor)},uShallowColor:{value:new THREE.Color(e.shallowColor)},uFoamColor:{value:new THREE.Color(e.foamColor)},uOpacityNear:{value:e.opacityNear},uFoamBreakThreshold:{value:e.foamBreakThreshold},uFoamCrestThreshold:{value:e.foamCrestThreshold},uFoamNoiseScale:{value:e.foamNoiseScale},uFoamDrift:{value:e.foamDriftSpeed},uFoamLag:{value:e.foamLag},uFoamTrail:{value:e.foamTrail},uFoamSheetLag:{value:e.foamSheetLag},uFoamSheetTrail:{value:e.foamSheetTrail},uDeepReference:{value:a.beach.maxDepth*.55}};for(let o=0;o<4;o++){const r=F[o],n=["x","y","z","w"][o];A.uOmega.value[n]=r?r.omega:0,A.uSteepness.value[n]=r?r.steepness:0,A.uKSin.value[n]=r?r.kSin:0}return t.onBeforeCompile=o=>{Object.assign(o.uniforms,A),o.vertexShader=o.vertexShader.replace("#include <common>",`#include <common>
`+J).replace("#include <beginnormal_vertex>",Q).replace("#include <begin_vertex>",$),o.fragmentShader=o.fragmentShader.replace("#include <common>",`#include <common>
`+ee).replace("#include <roughnessmap_fragment>",ae).replace("#include <map_fragment>",`#include <map_fragment>
`+te)},t.customProgramCacheKey=()=>"ocean-water-1",t}function de(){const{rows:a,cols:e}=N,t=a*e;if(!b){b={phase:new Float32Array(t*4),amp:new Float32Array(t*4),waveK:new Float32Array(t*4),shore:new Float32Array(t*4),sharp:new Float32Array(t*4)};for(const o of["phase","amp","waveK","shore","sharp"]){const r=new THREE.BufferAttribute(b[o],4);r.setUsage(THREE.DynamicDrawUsage),E.setAttribute("a"+o.charAt(0).toUpperCase()+o.slice(1),r)}}oe()}function oe(){const{rows:a,cols:e}=N,t=F.length,o=Math.PI*2,r=tideOffset(H,D.water);for(let n=0;n<a;n++){const i=n*e*4,m=u.phase[n*t],h=t>1?u.phase[n*t+1]:0,p=t>2?u.phase[n*t+2]:0,s=t>3?u.phase[n*t+3]:0,w=[m-Math.floor(m/o)*o,h-Math.floor(h/o)*o,p-Math.floor(p/o)*o,s-Math.floor(s/o)*o];for(let y=0;y<e;y++){const d=i+y*4;for(let f=0;f<4;f++)b.phase[d+f]=w[f],b.amp[d+f]=f<t?u.amp[n*t+f]:0,b.sharp[d+f]=f<t?u.sharp[n*t+f]:0,b.waveK[d+f]=f<t?u.k[n*t+f]:1;b.shore[d]=u.depth[n],b.shore[d+1]=u.breaking[n],b.shore[d+2]=u.foamBed[n],b.shore[d+3]=r}}for(const n of["aPhase","aAmp","aWaveK","aShore","aSharp"]){const i=E.getAttribute(n);i&&(i.needsUpdate=!0)}me()}function me(){const{rows:a,cols:e}=N;let t=E.getAttribute("color");t||(t=new THREE.BufferAttribute(new Float32Array(a*e*4),4),t.setUsage(THREE.DynamicDrawUsage),E.setAttribute("color",t),R.vertexColors=!0,R.needsUpdate=!0);const o=t.array;for(let r=0;r<a;r++){const n=u.edge[r];for(let i=0;i<e;i++){const m=(r*e+i)*4;o[m]=1,o[m+1]=1,o[m+2]=1,o[m+3]=n}}t.needsUpdate=!0}export function updateWater(a){if(!v)return;const e=Math.max(0,Math.min(.25,a));H+=e,A&&(A.uTime.value=H),L+=e;const t=1/Math.max(1,D.water.profileHz);L>=t&&(L=0,buildProfile(M,H,D,u),oe()),ne()}function ne(){const a=breakRow(u);if(a<0){O.fill(null);return}const e=F.length,t=Math.PI*2;for(let o=0;o<Math.min(_,e);o++){const r=u.phase[a*e+o]-F[o].omega*H,n=Math.floor(r/t),i=O[o];if(O[o]=n,i===null||Math.abs(n-i)>2||n>=i)continue;const m=u.amp[a*e+o],h=re(a),p=C(.25+1.6*(h>g?m/h:0)*u.breaking[a],0,1),s=Math.sin(r*.37+o*2.1)*.6;B.push({strength:p,pan:s,row:a,distance:D.beach.shoreZ-M[a]})}}function re(a){const e=F.length;let t=0;for(let o=0;o<e;o++)t+=u.amp[a*e+o];return t}export function consumeBreaks(){const a=B;return B=[],a}export function breakDistance(){if(!u)return 0;const a=breakRow(u);return a<0?0:D.beach.shoreZ-M[a]}export function getWaterMesh(){return v}export function getProfile(){return u}export function getElapsed(){return H}export function disposeWater(){v&&v.parent&&v.parent.remove(v),E&&E.dispose(),R&&R.dispose(),v=null,E=null,R=null,A=null,b=null,u=null,M=null,N=null,F=null,O=null,B=[],H=0,L=0}export const __test__={GRAVITY:U,SOUNDING_WAVES:_,clamp:C,smoothstep:X,totalAmpAt:re,detectBreaks:ne,VERTEX_HEAD:J,VERTEX_BODY:Q,VERTEX_POSITION:$,FRAGMENT_HEAD:ee,FRAGMENT_BODY:te,FRAGMENT_ROUGHNESS:ae,setElapsed:a=>{H=a},state:()=>({grid:N,profile:u,constants:F,rowZ:M,uniforms:A,attributes:b,cycles:O})};
