import{OCEAN_CONFIG as y}from"./config.min.js";const _=9.81,v=1e-6,U=2;export function bedHeightAt(a,t=y.beach){return Math.max(-t.maxDepth,(a-t.shoreZ)*t.slope)}export function tideOffset(a,t=y.water){return t.tideRange/2*Math.sin(Math.PI*2*a/t.tidePeriodSeconds)}export function depthAt(a,t,e=y.beach,o=y.water){const r=t-bedHeightAt(a,e);return r<=0?0:Math.max(o.minDepth,r)}export function waveNumberAt(a,t){if(t<=v)return a;const e=Math.tanh(a*t);return a/Math.sqrt(Math.max(e,v))}export function shoalingAt(a,t,e){if(t<=v||a<=v)return 1;const o=a*t,r=o>10?.5:.5*(1+2*o/Math.sinh(2*o)),n=Math.sqrt(_/a*Math.tanh(o));return Math.sqrt(e/Math.max(2*r*n,v))}export function sharpenAt(a,t,e,o=y.water.crestSharpen){if(a<=v||t<=v||e<=v)return 0;const r=Math.tanh(t*e);if(r<=v)return o;const n=a*t/4*(3/(r*r*r)-1/r);return Math.min(o,n)}export function breakAmount(a,t,e=y.water){if(t<=v)return 1;const o=a/t,r=e.breakRatio-e.breakSoftness,n=e.breakRatio+e.breakSoftness;return Z(r,n,o)}export function envelopeAt(a,t,e=y.water){const o=t*7.3,r=Math.sin(Math.PI*2*(a+o)/e.setPeriodSeconds),n=Math.sin(Math.PI*2*(a+o)/e.setSubPeriodSeconds);return 1+e.setDepth*r*n}export function rowPositions(a,t=y){const{beach:e,camera:o,water:r}=t,n=new Float32Array(a),s=Math.max(0,Math.min(e.nearRows,a-2)),m=a-s,h=o.z-e.nearZ,p=o.z-e.farZ;for(let i=0;i<s;i++)n[i]=o.z-(h+(e.rowNear-h)*(i/s));for(let i=0;i<m;i++){const w=m>1?i/(m-1):0;n[s+i]=o.z-(e.rowNear+(p-e.rowNear)*Math.pow(w,r.rowBias))}return n}export function halfWidthAt(a,t=y.beach,e=y.camera){return t.baseHalfWidth+Math.max(0,e.z-a)*t.widthPerMetre}export function waveConstants(a){return a.map(t=>{const e=Math.PI*2/t.length,o=Math.sqrt(_/e),r=Math.max(-.95,Math.min(.95,t.dirX));return{k0:e,c0:o,omega:e*o*t.speed,kSin:e*r,amplitude:t.amplitude,steepness:t.steepness}})}export function buildProfile(a,t,e=y,o=null){const{beach:r,water:n}=e,s=a.length,m=waveConstants(n.waves),h=m.length,p=tideOffset(t,n),i=o||{depth:new Float32Array(s),breaking:new Float32Array(s),foamBed:new Float32Array(s),edge:new Float32Array(s),phase:new Float32Array(s*h),amp:new Float32Array(s*h),sharp:new Float32Array(s*h),k:new Float32Array(s*h)},w=[];for(let c=0;c<h;c++)w.push(envelopeAt(t,c,n));const x=4+40*n.foamPersistence,d=new Float32Array(h),f=new Float32Array(h);let T=0,C=0,q=a[s-1];for(let c=s-1;c>=0;c--){const G=a[c],X=G-q;q=G;const F=depthAt(G,p,r,n);if(i.depth[c]=F,F<=0){i.breaking[c]=0,i.foamBed[c]=0,i.edge[c]=0;for(let l=0;l<h;l++)i.phase[c*h+l]=d[l],i.amp[c*h+l]=0,i.sharp[c*h+l]=0,i.k[c*h+l]=m[l].k0;continue}let z=0;for(let l=0;l<h;l++){const E=m[l],I=waveNumberAt(E.k0,F);i.k[c*h+l]=I;const j=P(E.kSin/I,-.95,.95),le=Math.sqrt(1-j*j);d[l]+=I*le*X,i.phase[c*h+l]=d[l];const ce=shoalingAt(I,F,E.c0);f[l]=E.amplitude*w[l]*(1+n.shoalGain*(ce-1)),z+=f[l]}const Y=Z(0,.35,F);i.edge[c]=Y;const K=n.breakRatio*F/2,se=smoothMin(z,K,K*.3)*Y,ie=z>v?se/z:1;let W=0;for(let l=0;l<h;l++){const E=f[l]*ie;i.amp[c*h+l]=E,i.sharp[c*h+l]=E*sharpenAt(E,i.k[c*h+l],F,n.crestSharpen),W+=E}const V=breakAmount(z*2,F,n);i.breaking[c]=V,C=Math.max(C,W);const he=C>v?W/C:0;T=Math.max(V*he,T*Math.exp(-Math.abs(X)/x)),i.foamBed[c]=T}return i}export function breakRow(a,t=.5){for(let e=a.breaking.length-1;e>=0;e--)if(a.depth[e]>0&&a.breaking[e]>=t)return e;return-1}function P(a,t,e){return a<t?t:a>e?e:a}export function smoothMin(a,t,e){if(e<=v)return Math.min(a,t);const o=P(.5+.5*(t-a)/e,0,1);return t*(1-o)+a*o-e*o*(1-o)}function Z(a,t,e){if(t<=a)return e>=t?1:0;const o=P((e-a)/(t-a),0,1);return o*o*(3-2*o)}const J=`
uniform float uTime;
uniform vec4 uOmega;
...
`;let g=null,N=null,S=null,A=null,H=null,k=null,u=null,M=null,D=null,b=null,R=0,L=0,B=[],O=null;export function initWater(a,t=y,e={}){H=t;const o=t.water,r=e.mobile?o.mobileScale:1,n=Math.max(8,Math.round(o.rows*r)),s=Math.max(8,Math.round(o.cols*r));return D={rows:n,cols:s},k=waveConstants(o.waves),M=rowPositions(n,t),u=buildProfile(M,0,t),O=new Array(U).fill(null),S=fe(n,s,M,t.beach),N=de(t,e.sky||null),me(),g=new THREE.Mesh(S,N),g.name="ocean",g.matrixAutoUpdate=!1,g.updateMatrix(),g.frustumCulled=!1,g.renderOrder=1,a&&a.add(g),g}function fe(a,t,e,o){const r=new THREE.BufferGeometry,n=a*t,s=new Float32Array(n*3),m=new Float32Array(n*3);for(let w=0;w<a;w++){const x=halfWidthAt(e[w],o);for(let d=0;d<t;d++){const f=t>1?d/(t-1):.5,T=(w*t+d)*3;s[T]=(f*2-1)*x,s[T+1]=0,s[T+2]=e[w],m[T+1]=1}}const h=n>65535?Uint32Array:Uint16Array,p=new h((a-1)*(t-1)*6);let i=0;for(let w=0;w<a-1;w++)for(let x=0;x<t-1;x++){const d=w*t+x,f=d+1,T=d+t,C=T+1;p[i++]=d,p[i++]=f,p[i++]=T,p[i++]=f,p[i++]=C,p[i++]=T}return r.setAttribute("position",new THREE.BufferAttribute(s,3)),r.setAttribute("normal",new THREE.BufferAttribute(m,3)),r.setIndex(new THREE.BufferAttribute(p,1)),r}function de(a,t=null){const e=a.water,o=new THREE.MeshStandardMaterial({color:16777215,roughness:.08,metalness:0,transparent:!0,side:THREE.DoubleSide});A={uTime:{value:0},uOmega:{value:new THREE.Vector4},uSteepness:{value:new THREE.Vector4},uKSin:{value:new THREE.Vector4},uLeanGain:{value:e.leanGain},uDeepColor:{value:new THREE.Color(e.deepColor)},uShallowColor:{value:new THREE.Color(e.shallowColor)},uFoamColor:{value:new THREE.Color(e.foamColor)},uOpacityNear:{value:e.opacityNear},uFoamBreakThreshold:{value:e.foamBreakThreshold},uFoamCrestThreshold:{value:e.foamCrestThreshold},uFoamNoiseScale:{value:e.foamNoiseScale},uFoamDrift:{value:e.foamDriftSpeed},uFoamLag:{value:e.foamLag},uFoamTrail:{value:e.foamTrail},uFoamSheetLag:{value:e.foamSheetLag},uFoamSheetTrail:{value:e.foamSheetTrail},uDeepReference:{value:a.beach.maxDepth*.55}};for(let r=0;r<4;r++){const n=k[r],s=["x","y","z","w"][r];A.uOmega.value[s]=n?n.omega:0,A.uSteepness.value[s]=n?n.steepness:0,A.uKSin.value[s]=n?n.kSin:0}return o.onBeforeCompile=r=>{Object.assign(r.uniforms,A),t&&Object.assign(r.uniforms,t.uniforms),r.vertexShader=r.vertexShader.replace("#include <common>",`#include <common>
`+J).replace("#include <beginnormal_vertex>",Q).replace("#include <begin_vertex>",$),r.fragmentShader=r.fragmentShader.replace("#include <common>",`#include <common>
`+ee+(t?t.uniformGlsl+t.glsl:"")).replace("#include <roughnessmap_fragment>",ae).replace("#include <map_fragment>",`#include <map_fragment>
`+te),t&&(r.fragmentShader=r.fragmentShader.replace("#include <opaque_fragment>",`#include <opaque_fragment>
`+ue))},o.customProgramCacheKey=()=>t?"ocean-water-1-sky":"ocean-water-1",o}function me(){const{rows:a,cols:t}=D,e=a*t;if(!b){b={phase:new Float32Array(e*4),amp:new Float32Array(e*4),waveK:new Float32Array(e*4),shore:new Float32Array(e*4),sharp:new Float32Array(e*4)};for(const o of["phase","amp","waveK","shore","sharp"]){const r=new THREE.BufferAttribute(b[o],4);r.setUsage(THREE.DynamicDrawUsage),S.setAttribute("a"+o.charAt(0).toUpperCase()+o.slice(1),r)}}oe()}function oe(){const{rows:a,cols:t}=D,e=k.length,o=Math.PI*2,r=tideOffset(R,H.water);for(let n=0;n<a;n++){const s=n*t*4,m=u.phase[n*e],h=e>1?u.phase[n*e+1]:0,p=e>2?u.phase[n*e+2]:0,i=e>3?u.phase[n*e+3]:0,w=[m-Math.floor(m/o)*o,h-Math.floor(h/o)*o,p-Math.floor(p/o)*o,i-Math.floor(i/o)*o];for(let x=0;x<t;x++){const d=s+x*4;for(let f=0;f<4;f++)b.phase[d+f]=w[f],b.amp[d+f]=f<e?u.amp[n*e+f]:0,b.sharp[d+f]=f<e?u.sharp[n*e+f]:0,b.waveK[d+f]=f<e?u.k[n*e+f]:1;b.shore[d]=u.depth[n],b.shore[d+1]=u.breaking[n],b.shore[d+2]=u.foamBed[n],b.shore[d+3]=r}}for(const n of["aPhase","aAmp","aWaveK","aShore","aSharp"]){const s=S.getAttribute(n);s&&(s.needsUpdate=!0)}pe()}function pe(){const{rows:a,cols:t}=D;let e=S.getAttribute("color");e||(e=new THREE.BufferAttribute(new Float32Array(a*t*4),4),e.setUsage(THREE.DynamicDrawUsage),S.setAttribute("color",e),N.vertexColors=!0,N.needsUpdate=!0);const o=e.array;for(let r=0;r<a;r++){const n=u.edge[r];for(let s=0;s<t;s++){const m=(r*t+s)*4;o[m]=1,o[m+1]=1,o[m+2]=1,o[m+3]=n}}e.needsUpdate=!0}export function updateWater(a){if(!g)return;const t=Math.max(0,Math.min(.25,a));R+=t,A&&(A.uTime.value=R),L+=t;const e=1/Math.max(1,H.water.profileHz);L>=e&&(L=0,buildProfile(M,R,H,u),oe()),ne()}function ne(){const a=breakRow(u);if(a<0){O.fill(null);return}const t=k.length,e=Math.PI*2;for(let o=0;o<Math.min(U,t);o++){const r=u.phase[a*t+o]-k[o].omega*R,n=Math.floor(r/e),s=O[o];if(O[o]=n,s===null||Math.abs(n-s)>2||n>=s)continue;const m=u.amp[a*t+o],h=re(a),p=P(.25+1.6*(h>v?m/h:0)*u.breaking[a],0,1),i=Math.sin(r*.37+o*2.1)*.6;B.push({strength:p,pan:i,row:a,distance:H.beach.shoreZ-M[a]})}}function re(a){const t=k.length;let e=0;for(let o=0;o<t;o++)e+=u.amp[a*t+o];return e}export function consumeBreaks(){const a=B;return B=[],a}export function breakDistance(){if(!u)return 0;const a=breakRow(u);return a<0?0:H.beach.shoreZ-M[a]}export function getWaterMesh(){return g}export function getProfile(){return u}export function getElapsed(){return R}export function disposeWater(){g&&g.parent&&g.parent.remove(g),S&&S.dispose(),N&&N.dispose(),g=null,S=null,N=null,A=null,b=null,u=null,M=null,D=null,k=null,O=null,B=[],R=0,L=0}export const __test__={GRAVITY:_,SOUNDING_WAVES:U,clamp:P,smoothstep:Z,totalAmpAt:re,detectBreaks:ne,VERTEX_HEAD:J,VERTEX_BODY:Q,VERTEX_POSITION:$,FRAGMENT_HEAD:ee,FRAGMENT_BODY:te,FRAGMENT_ROUGHNESS:ae,setElapsed:a=>{R=a},state:()=>({grid:D,profile:u,constants:k,rowZ:M,uniforms:A,attributes:b,cycles:O})};
