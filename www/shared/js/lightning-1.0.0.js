...
/**
 * lightning-1.0.0.js - The strikes, the flash they throw, and the bolts they
 * draw.
 *
 * PROMOTED FROM www/highwater/js/lightning.js ON 2026-09-23 so Tornado Alley
 * could have High Water's streaks, and High Water now runs on it: a
 * frame-by-frame comparison of the flash, the light and every bolt buffer
 * against High Water's own copy showed nothing changed on the way. The notes
 * below are High Water's, written while the lightning was tuned there, so
 * "the sea" in them is its sea.
 *
 * ---- WHAT A SCENE HANDS IT ----
 *
 * Its settings in High Water's shape: a config with `storm.seconds` (the
 * story's length, which the curves are clamped to), `storm.lightning` (every
 * number below; see www/highwater/js/config.js for the tuned set and its
 * reasons, and www/tornado/js/config.js for a second), and `camera.fov`. There
 * is no default: every function that reads settings takes them.
 *
 * And, through initLightning's `options.sky.uniforms`, the object the flash
 * is written into: `uFlash` (a brightness) and `uFlashDir` (a unit vector
 * toward the strike). The scene's own shaders decide what a flash lights. The
 * part adds a directional light for anything lit by three's lights, and draws
 * the bolts itself.
 *
 * Anything that flashes on a story clock should also ask the shared player's
 * `flashAllowed()` before each update: a scrub runs the clock far faster than
 * real time, and the rate cap below is written in story seconds.
 *
...
// ---- The curve reader, High Water's storm.js own -------------------------------
 ...
 *  clamped at both ends and to the story's length. High Water's storm.js
 *  curveAt, exactly, so its strike rate is the one it was tuned on. */
export function curveAt(seconds, keys, storm) {
...
export function strikeRateAt(seconds, config) {
...
export function flashesPerSecondCeiling(config) {
...
export function boltAzimuthLimit(aspect, config, fovDegrees = null) {
...
export function strikeDirection(azimuth, distance, config) {
...
export function generateBolt(from, to, random = Math.random, config) {
...
export function strikeEndpoints(azimuth, distance, config) {
    const cfg = config.storm.lightning.bolt;
...
export function initLightning(scene, camera, config, options = {}) {
    settings = config;
    sceneRef = scene;
...
export function updateLightning(seconds, config) {
