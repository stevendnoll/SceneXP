...
| `pan.js` | Portrait view controls for view-only experiences (bounded yaw + FOV zoom of the fixed camera), plus touch gestures on the scene: swipe to pan and tilt, pinch to zoom (full WASD on keyboards: A/D pan, W/S tilt, Shift+Up/Down tilt too); an optional `zoomDelegate` reroutes every zoom input (buttons, keys, pinch as log2 of spread) to the experience as signed deltas instead of FOV, with `limits()` polled to dim the buttons (mandelbrot's infinite dive) | `initPortraitControls({ getCamera, lookAt, baseFov, pan: { speed, maxAngle, maxTilt }, zoom: { speed, maxIn, maxOut }, zoomDelegate: { onDelta, limits }, surface, extraClass, onFirstUse, signal })`, then `updatePortraitControls(dt)` each frame; tap handlers on the same surface check `gestureClaimedTap()` |
