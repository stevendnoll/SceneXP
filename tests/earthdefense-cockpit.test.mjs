...
/** How far off the nose a piece's NEAREST edge sits, in degrees, measured at
 *  the canopy plane. Vertical for the dash and the glow strip, horizontal for
 *  the struts, because that is the direction each of them encroaches from. */
function clearanceDegrees(mesh, axis) {
    const z = Math.abs(mod.__test__.CANOPY_Z);
    const half = (axis === 'y' ? mesh.scale.y : mesh.scale.x) / 2;
    const centre = axis === 'y' ? mesh.position.y : mesh.position.x;
    const nearestEdge = Math.abs(centre) - half;
    return Math.atan(Math.max(0, nearestEdge) / z) * DEG;
}

describe('the canopy keeps the centre of the screen clear', () => {
    // Twice the six degree targeting cone. The reticle and everything it can
    // lock onto sit inside this, in clear space.
    const CLEAR_CONE = 12;
    ...
    for (const [name, aspect] of Object.entries(ASPECTS)) {
        test(`on ${name}`, () => {
            ...
            expect(clearanceDegrees(byName('dash'), 'y')).toBeGreaterThan(CLEAR_CONE);
            expect(clearanceDegrees(byName('indicator-glow'), 'y')).toBeGreaterThan(CLEAR_CONE);
            expect(clearanceDegrees(byName('strut-0'), 'x')).toBeGreaterThan(CLEAR_CONE);
            expect(clearanceDegrees(byName('strut-1'), 'x')).toBeGreaterThan(CLEAR_CONE);
        });
    }
    ...
    test('portrait is the tightest case, which is why it is designed for first', () => {
        ...
        const portrait = clearanceDegrees(byName('strut-0'), 'x');
        mod.resizeCockpit(ASPECTS['a wide desktop']);
        const desktop = clearanceDegrees(byName('strut-0'), 'x');
...
describe('the canopy does not obscure the opening frame', () => {
    test("Earth's limb sits well above the dash at spawn", () => {
        ...
        // what it is for.
        expect(clearanceDegrees(byName('dash'), 'y')).toBeGreaterThan(limbBelowNose + 5);
    });
...
describe('lifecycle', () => {
    test('init returns an overlay scene holding one canopy group', () => {
        ...
        // Dash, two struts, and the indicator glow.
        expect(mod.getCanopy().children).toHaveLength(4);
