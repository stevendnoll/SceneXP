...
const { XO_CONFIG: CFG, FIELD } = await import(join(scene, 'config.js'));
...
describe('500, the perfect game', () => {
    ...
    test('the fans arrive in a ripple around the regulars, and all of them arrive', () => {
        const { seats, cols } = St.crowdSeats();
        const arriving = seats.filter((seat) => !seat.regular);
        // Not vacuous: the perfect game still has a stand's worth to bring in.
        expect(arriving.length).toBeGreaterThan(seats.length * 0.3);
        for (const seat of seats) {
            // A regular was there all along, so never pops in a second time.
            const before = seat.regular ? 1 : 0;
            expect(St.fanArrival(F.fill[0] - 0.01, seat, cols)).toBe(before);
